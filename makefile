...
clear:
	...
	var=`ipcs | awk '/^------ Shared Memory Segments --------$$/,/^------ Semaphore Arrays --------$$/' | grep '^0x[0-9,a-f]\+[^0-9,^a-z][0-9]\+[^0-9,^a-z]\+[a-z]\+[^0-9,^a-z]\+[0-9]\+[^0-9,^a-z]\+4[^0-9,^a-z].*$$'| sed 's/^0x[0-9,a-f]\+[^0-9,^a-f]\([0-9]\+\)[^0-9,^a-f].*$$/\1/g'`;for i in $$var; do ipcrm -m $$i;done
	var=`ipcs | awk '/^------ Shared Memory Segments --------$$/,/^------ Semaphore Arrays --------$$/' | grep '^0x[0-9,a-f]\+[^0-9,^a-z][0-9]\+[^0-9,^a-z]\+[a-z]\+[^0-9,^a-z]\+777+[^0-9,^a-z]\+[0-9]\+[^0-9,^a-z].*$$'| sed 's/^0x[0-9,a-f]\+[^0-9,^a-f]\([0-9]\+\)[^0-9,^a-f].*$$/\1/g'`;for i in $$var; do ipcrm -m $$i;done
